...
def StartTests(ip,id,url,test1,test2,test3,test4,test5,test6):
    ...
    if  test3:
        os.popen(f'sudo docker run -d test3 argusproof/argusproof_deltateam:python3 blacklistcheck.py {url} {id}')
    if  test4:
        os.popen(f'sudo docker run -d test4 argusproof/argusproof_deltateam:python3 sslcheck.py {url} {id}')
    if  test5:
