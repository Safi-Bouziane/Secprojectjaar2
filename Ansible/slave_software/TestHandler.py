...
def StartTests(ip,id,url,test1,test2,test3,test4,test5,test6,test7,test8):
    ...
    if  test7:
        os.popen(f'sudo docker run -d argusproof/argusproof_deltateam:test7 python3 dmarccheck.py {url} {id}')
    if  test8:
        os.popen(f'sudo docker run -d argusproof/argusproof_deltateam:test8 python3 spfcheck.py {url} {id}')
